...
class _SearchScreenState extends SafeState<SearchScreen> {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          GestureDetector(
            onTap: () {
              // navigate to profile based on account type
              final auth = context.read<AuthProvider>();
              if (auth.currentAccountType == AccountType.owner) {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => OwnerShell(initialIndex: 3)),
                );
              } else {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => UserShell(initialIndex: 3)),
                );
              }
            },
            child: Container(
